...
def getTrait(traitList, textFilter = lambda x: False):
    conflictItems = []
    selectedTrait = random.choice(traitList)
    while textFilter(selectedTrait):
        selectedTrait = random.choice(traitList)
...
@bot.command(name = 'groupspin',
    description = 'Spin the trait wheel, but only for group traits'
    , aliases = ['group'])
async def groupSpin(ctx):
    traits = listdir('./group')
    chosenTrait = random.choice(traits)
    with open('./group/' + chosenTrait, 'r') as file:
        traitText = [l.strip() for l in file.readlines()]
    
    await printTrait({ 'trait': chosenTrait.replace('.txt', ''), 'text': traitText }, ctx)

async def sharedSpin(ctx):
    traits = listdir('./traits')
    traitDict = getTrait(traits, textFilter=lambda x: 'Shared Modifier:' not in x)
    await printTrait(traitDict, ctx)
